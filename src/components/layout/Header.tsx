...
export function Header({
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/95 dark:bg-slate-800/95 md:bg-white/80 md:dark:bg-slate-800/50 md:backdrop-blur-md border-b border-slate-200 dark:border-slate-700 safe-area-top">
      ...
      {isMobileToolsOpen && (
        <div className="md:hidden border-t border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-800/95 p-2">
          {/* Tools Icons Row */}
          <div className="flex flex-wrap justify-center gap-2">
            <button 
              onClick={() => {
                if (authToken) { onAddLink(); setIsMobileToolsOpen(false); }
                else { onOpenAuth(); setIsMobileToolsOpen(false); }
              }} 
              className={`flex items-center justify-center w-10 h-10 rounded-lg ${authToken ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' : 'bg-slate-50 dark:bg-slate-700 text-slate-400 dark:text-slate-500'}`}
              title={authToken ? "添加链接" : "登录"}
            >
              <Plus size={18} />
            </button>
            <button 
              ...
              className={`flex items-center justify-center w-10 h-10 rounded-lg ${authToken ? 'bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300' : 'bg-slate-50 dark:bg-slate-700 text-slate-400 dark:text-slate-500'}`}
              title={authToken ? "分类管理" : "登录"}
            >
              <FolderTree size={18} />
            </button>
            <button onClick={() => { onOpenSettings(); setIsMobileToolsOpen(false); }} className="flex items-center justify-center w-10 h-10 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300" title="设置">
              <Settings size={18} />
            </button>
            <button onClick={() => { onOpenBackup(); setIsMobileToolsOpen(false); }} className="flex items-center justify-center w-10 h-10 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300" title="备份">
              <Upload size={18} />
            </button>
            <button 
              onClick={() => {
              ...
              className={`flex items-center justify-center w-10 h-10 rounded-lg ${isDragSortMode ? 'bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400' : authToken ? 'bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300' : 'bg-slate-50 dark:bg-slate-700 text-slate-400 dark:text-slate-500'}`}
              title={authToken ? (isDragSortMode ? "完成排序" : "拖动排序") : "登录"}
            >
              <GripVertical size={18} />
            </button>
            <button 
              ...
              className={`flex items-center justify-center w-10 h-10 rounded-lg ${isEditMode ? 'bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400' : authToken ? 'bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300' : 'bg-slate-50 dark:bg-slate-700 text-slate-400 dark:text-slate-500'}`}
              title={authToken ? (isEditMode ? "完成编辑" : "编辑模式") : "登录"}
            >
              <Pencil size={18} />
            </button>
            <button 
              ...
              className={`flex items-center justify-center w-10 h-10 rounded-lg ${isBatchEditMode ? 'bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400' : authToken ? 'bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300' : 'bg-slate-50 dark:bg-slate-700 text-slate-400 dark:text-slate-500'}`}
              title={authToken ? (isBatchEditMode ? "完成批量" : "批量编辑") : "登录"}
            >
              <CheckSquare size={18} />
            </button>
            {authToken ? (
              <button onClick={() => { onLogout(); setIsMobileToolsOpen(false); }} className="flex items-center justify-center w-10 h-10 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400" title="退出">
                <LogOut size={18} />
              </button>
            ) : (
              <button onClick={() => { onOpenAuth(); setIsMobileToolsOpen(false); }} className="flex items-center justify-center w-10 h-10 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400" title="登录">
                <Lock size={18} />
              </button>
            )}
            <a
              ...
              className="flex items-center justify-center w-10 h-10 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300"
              title="GitHub"
            >
